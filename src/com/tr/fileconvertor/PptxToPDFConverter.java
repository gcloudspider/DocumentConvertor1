...
public class PptxToPDFConverter extends Converter{
	...
	protected Dimension processSlides() throws IOException{
		...
		XMLSlideShow v_ppt = new XMLSlideShow(iStream);
		Dimension dimension = v_ppt.getPageSize();
		XSLFSlide[] tempSlides = new XSLFSlide[v_ppt.getSlides().size()];
		slides = v_ppt.getSlides().toArray(tempSlides);
		return dimension;
	}
